...
for d in $DATA_DIR/*; do
	...
	if [ -d "$d" ]; then
        ...
        NTASKS=$((NTASKS<2048 ? NTASKS : 2048))
        # Edit the number of nodes requested in the SBATCH headers
        sed --i -e "s/#SBATCH --ntasks=[0-9]\+/\#SBATCH --ntasks=$NTASKS/" hpc/greasy/greasy_scheduler.job
        # Send job to queue using greasy
        sbatch hpc/greasy/greasy_scheduler.job $WORKING_DIR/hpc/greasy/tasks/tasks_file_$BASENAME.txt
        # Display some info about the job that has been sent
        echo Job sent for: $BASENAME ($NTASKS tasks on $NNODES nodes)
      else
